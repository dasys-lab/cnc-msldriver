    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ROSCommunicator::getInstance(), SIGNAL(receivedSettings(msl_sensor_msgs::CameraSettings::ConstPtr)), this,
            SLOT(handleReceivedSettings(msl_sensor_msgs::CameraSettings::ConstPtr)));
...
MainWindow::~MainWindow()
{
    ...
    free(m_lookupTable);
    auto it = this->m_robots.begin();
    while (it != this->m_robots.end())
    {
        auto keyCopy = it->first;
        delete it->second;
        it = this->m_robots.erase(it);
        delete keyCopy;
    }
...
void MainWindow::processFilter()
{
    ...
    if (index > -1)
    {
        ...
        filterYUVExtractSubImages.process(content, imgRes->getWidth(), imgRes->getHeight(), mx, my, image_gray, image_uv, image_roi, image_roiRoland,
                                          imbrightenedGray);
        free(content);
...
void MainWindow::addImage(ImageResource *imgRes, bool show)
{
    ...
    ui->selectImageComboBox->addItem(QString("%1 %2").arg(QString(buffer)).arg(QString::fromStdString(imgRes->m_filename)));
...
void MainWindow::writeLookupTableInterpolate()
{
    ...
    AbstractCommand *command =
        new CommandInterpolateYUV2Lookuptable(getCurrentLookupTable(), polygon, ui->emThresholdSpinBox->value(), ui->emClusterSpinBox->value());
    ;
...
void MainWindow::saveAllImagesToFile(QString suffix)
{
    if (m_images.size() > 0)
    {
        QString qfilename = QFileDialog::getExistingDirectory(this, tr("Select a Directory"), m_imagePwd);
        if (!qfilename.isNull())
        {
...
{
    AbstractCommand *command =
        new CommandThresholds2Lookuptable(getCurrentLookupTable(), ui->lowerThresholdSpinBox->value(), ui->upperThresholdSpinBox->value());
    if (command->doIt())
    {
        m_commandList->addCommand(command);
    }
...
void MainWindow::on_saveImageToFileAction_triggered()
{
    ...
    if (index > -1)
    {
        ...
        QString sf;
        QString qfilename = QFileDialog::getSaveFileName(this, tr("Save File"), m_imagePwd, tr("RAW (*.raw);; PNG (*.png)"), &sf);
        if (qfilename != NULL)
        {
            QFileInfo f(qfilename);
            if (f.suffix().isEmpty())
...
{
    QString qfilename = QFileDialog::getOpenFileName(this, tr("Open File"), getLookupTablePwd(), tr("TXT (*.txt);; All files (*.*)"));
    if (qfilename != NULL)
    {
...
252 57 176 43
*/
                if (img_x >= 164 * 2 && img_x <= (164 + 54) * 2 && img_y >= 173 && img_y <= 173 + 126)
                {
                    lookupTable[u * 256 + v] = 255;
                }
                else if (img_x >= 197 * 2 && img_x <= (197 + 83) * 2 && img_y >= 211 && img_y <= 211 + 147)
                {
                    lookupTable[u * 256 + v] = 255;
                }
                else if (img_x >= 257 * 2 && img_x <= (257 + 61) * 2 && img_y >= 284 && img_y <= 284 + 115)
                {
                    lookupTable[u * 256 + v] = 255;
                }
                else if (img_x >= 395 * 2 && img_x <= (395 + 99) * 2 && img_y >= 212 && img_y <= 212 + 100)
                {
                    lookupTable[u * 256 + v] = 255;
                }
                else if (img_x >= 355 * 2 && img_x <= (355 + 66) * 2 && img_y >= 284 && img_y <= 284 + 114)
                {
                    lookupTable[u * 256 + v] = 255;
                }
                else if (img_x >= 400 * 2 && img_x <= (400 + 62) * 2 && img_y >= 290 && img_y <= 290 + 87)
                {
                    lookupTable[u * 256 + v] = 255;
                }
                else if (img_x >= 209 * 2 && img_x <= (209 + 258) * 2 && img_y >= 84 && img_y <= 84 + 60)
                {
                    lookupTable[u * 256 + v] = 255;
                }
                else if (img_x >= 252 * 2 && img_x <= (252 + 176) * 2 && img_y >= 57 && img_y <= 57 + 43)
                {
                    lookupTable[u * 256 + v] = 255;
                }
            }
