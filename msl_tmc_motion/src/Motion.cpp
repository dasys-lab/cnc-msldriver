...
void Motion::initialize()
{
    ...
    this->logTypesAvailable = make_shared<vector<string>>(
        initializer_list<string>{"RPM", "PWM", "RPMGOAL", "CURRENT", "MGOAL", "MREQUEST", "MAXPWM", "ERRORINT", "MOTION", "MSMOOTH", "RPMSMOOTH"});
...
void Motion::log_goalie()
{
    ...
    if (isLogging)
    {
        fprintf(lp, "%f\t%f\t%f\t%f\t%f\t%f\t\n", rawOdoInfo.position.angle, rawOdoInfo.position.x, rawOdoInfo.position.y, rawOdoInfo.motion.angle,
                rawOdoInfo.motion.rotation, rawOdoInfo.motion.translation);
    }
}
...
bool Motion::open()
{
    ...
    if (!my_serial->isOpen())
    {
        cerr << "TMC-Motion: Unable to open serial port : " << this->device << " Errno: " << strerror(errno) << endl;
        return false;
    }
...
void Motion::sendMotorConfig()
{
    ...
    for (int i = 0; i < this->logTypesAvailable->size(); i++)
    {
        ...
        configPacket = make_shared<CNMCPacketConfigure>();
        configPacket->setData(CNMCPacket::ConfigureCmd::SetLogMode, make_shared<vector<int8_t>>((int8_t)i, logmode));
        this->sendData(configPacket);
        this->checkSuccess(configPacket);
    }
...
void Motion::getMotorConfig()
{
    ...
    this->mc.linFactor = (*sc)["Motion"]->tryGet<double>(0.0, "Motion", "CNMC", "Controller", "LinearFactor", NULL);
    this->mc.smoothFactor = (*sc)["Motion"]->tryGet<double>(1.0, "Motion", "CNMC", "Controller", "SmoothFactor", NULL);

    this->mc.maxErrorInt = (*sc)["Motion"]->tryGet<short>(1000, "Motion", "CNMC", "Controller", "MaxErrorInt", NULL);

    this->mc.rotationErrorWeight = (*sc)["Motion"]->get<double>("Motion", "CNMC", "Controller", "RotationErrorWeight", NULL);
    this->mc.rotationErrorByVeloWeight = (*sc)["Motion"]->get<double>("Motion", "CNMC", "Controller", "RotationErrorByVeloWeight", NULL);
    this->mc.rotationErrorByAccelWeight = (*sc)["Motion"]->get<double>("Motion", "CNMC", "Controller", "RotationErrorByAccelWeight", NULL);

    this->mc.deadBand = (*sc)["Motion"]->get<short>("Motion", "CNMC", "Controller", "DeadBand", NULL);

    this->mc.failSafeRPMBound = (*sc)["Motion"]->get<short>("Motion", "CNMC", "Controller", "FailSafeRPMBound", NULL);
    this->mc.failSafePWMBound = (*sc)["Motion"]->get<short>("Motion", "CNMC", "Controller", "FailSafePWMBound", NULL);
    this->mc.failSafeCycles = (*sc)["Motion"]->get<short>("Motion", "CNMC", "Controller", "FailSafeCycles", NULL);

    this->mc.accelBoundMin = (*sc)["Motion"]->tryGet<double>(0.0, "Motion", "CNMC", "Controller", "MinimalAccelerationAllowed");

    this->mc.accelBoundMax = (*sc)["Motion"]->tryGet<double>(0.0, "Motion", "CNMC", "Controller", "MaximalAccelerationAllowed");

    this->mc.rotationAccelBound = (*sc)["Motion"]->tryGet<double>(0.0, "Motion", "CNMC", "Controller", "MaxRotationAccel");

    this->mc.currentErrorBound = (*sc)["Motion"]->tryGet<short>(0, "Motion", "CNMC", "Controller", "CurrentErrorBound", NULL);
    ...
    this->mc.denominator = (*sc)["Motion"]->get<short>("Motion", "CNMC", "Motors", "ThermalMotorConstantDenominator", NULL);
    this->mc.numerator = (*sc)["Motion"]->get<short>("Motion", "CNMC", "Motors", "ThermalMotorConstantNumerator", NULL);

    int delta = (int)(((2.0 * M_PI * (double)mc.wheelRadius) / (((double)mc.resolution) * (double)mc.gearReduction)) * 1000.0);
...
void Motion::calcOdoPosition()
{
    ...
    geometry::CNPointAllo lastPos = geometry::CNPointAllo(lastOdoInfo.position.x, lastOdoInfo.position.y);

    // determine time driven
    ros::Time currTime = ros::Time::now();
    uint64_t currNanoSeconds = (currTime.sec * 1000000000UL + currTime.nsec);

    double timeSinceLastOdo = (double)(currNanoSeconds - lastOdoInfo.timestamp) / 1000000000.0;
    ...
    geometry::CNVecAllo translation = geometry::CNVecAllo(transX, transY);
    geometry::CNPointAllo middle;
    geometry::CNPointAllo newPos;
    ...
    if (rot != 0)
    {
        ...
        geometry::CNVecAllo radiusVect;

        if (rot < 0)
        {
            radiusVect = translation.normalize().rotateZ(M_PI / 2) * radiusLength;
        }
        else
        {
            radiusVect = translation.normalize().rotateZ(-M_PI / 2) * radiusLength;
        ...
        newPos = middle + radiusVect.rotateZ(angleDriven);
    }
    else
    {
    ...
    // update position
    rawOdoInfo.position.x = newPos.x;
    rawOdoInfo.position.y = newPos.y;
...
void Motion::run()
{
    ...
    while (Motion::running)
    {
        ...
        if (request == nullptr)
        {
            ...
            if (requestOld != nullptr && chrono::duration_cast<chrono::milliseconds>(std::chrono::steady_clock::now() - lastCommandTimestamp).count() > 250)
            {
                requestOld = nullptr;
            }
        ...
        long sleepTime = this->minCycleTime - chrono::duration_cast<chrono::milliseconds>(std::chrono::steady_clock::now() - this->cycleLastTimestamp).count();
...
void Motion::executeRequest(MotionSet *ms)
{
    ...
    packet->setData(CNMCPacket::ControlCmd::SetMotionVector, (short)(cos(ms->angle) * trans), (short)(sin(ms->angle) * trans), (short)(rot * 64));
