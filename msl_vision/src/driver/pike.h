#ifndef CAMERA_Pike_H
#define CAMERA_Pike_H 1
...
#define DC1394_ERR_OUT(err,message)                     \
  if (err!=DC1394_SUCCESS) {                            \
    ...
    class Pike : public Camera1394
    {
        public:
            /**
             * Setect the camera (let dc1394 enumerate the cameras on the bus)
             * @param index Pike to use [0..avail-1]
             */
            Pike(uint32_t index = 0);
    ...
    void DummyTestMethod();
        ...
        protected:
            ...
            void pikeInitInternal(uint32_t index);
