...
namespace camera
{
    ...
    void Guppy::enableHDR(bool enable, uint32_t knee1, uint32_t knee2)
    {
        uint32_t reg;
        dc1394error_t err;
...
#define REG_CAMERA_AVT_KNEEPOINT_2 0x288U
    ...
    Guppy::version_t Guppy::getVersion()
    {
        version_t version;
        dc1394error_t err;
    ...
    void Guppy::setWhiteBalance(const white_balance_t wb)
    {
    ...
    Guppy::white_balance_t Guppy::getWhiteBalance()
    {
    ...
    void Guppy::enableAutoShutter(bool value, uint32_t min, uint32_t max)
    {
    ...
    void Guppy::enableAutoGain(bool value, uint32_t min, uint32_t max)
    {
...
//by me
    ...
    void Guppy::enableMirror(bool value)
    {
        dc1394error_t err;
    ...
    Guppy::lut_info_t Guppy::getLUTInfo()
    {
        lut_info_t lutinfo;
        dc1394error_t err;
    ...
    void Guppy::enableLUT(bool value)
    {
    ...
    std::vector<uint8_t> Guppy::getLUT()
    {
        dc1394error_t err;
    ...
    void Guppy::setLUT(std::vector<uint8_t> &lut)
    {
    ...
    void Guppy::enableTestImage(bool enable, uint32_t number)
    {
        dc1394error_t err;
    ...
    void Guppy::softReset()
    {
        ...
        DC1394_ERR_THROW(err, "uanble to soft reset AVT Guppy");

        sleep(5);

        reset();

        guppyInitInternal(this->index);
        init();

        startCapture();
    }

    void Guppy::initInternal()
        throw(CameraException)
    {
        if (!this->dc_camera)
        {
    ...
    void Guppy::startCaptureInternal()
        throw(CameraException)
    {
    ...
    void Guppy::stopCaptureInternal()
        throw(CameraException)
    {
        if (!this->dc_camera) return;
    ...
    bool Guppy::getFrameInternal(Frame &frame)
    {
