#ifndef CAMERA_GUPPY_H
#define CAMERA_GUPPY_H 1
...
#define DC1394_ERR_OUT(err,message)                     \
  if (err!=DC1394_SUCCESS) {                            \
    ...
    class Guppy : public Camera1394
    {
        public:
            /**
             * Setect the camera (let dc1394 enumerate the cameras on the bus)
             * @param index Guppy to use [0..avail-1]
             */
            Guppy(uint32_t index = 0);
    ...
    void DummyTestMethod();
        ...
        protected:
            ...
            void guppyInitInternal(uint32_t index);
