...
namespace camera
{
    ...
    void Pike::enableHDR(bool enable, uint32_t knee1, uint32_t knee2)
    {
        uint32_t reg;
        dc1394error_t err;
...
#define REG_CAMERA_AVT_KNEEPOINT_2 0x288U
    ...
    Pike::version_t Pike::getVersion()
    {
        version_t version;
        dc1394error_t err;
    ...
    void Pike::opAutoWhiteBalance()
    {
        setFeatureMode(DC1394_FEATURE_WHITE_BALANCE,
                       DC1394_FEATURE_MODE_ONE_PUSH_AUTO);
    }

    void Pike::setWhiteBalance(const white_balance_t wb)
    {
    ...
    Pike::white_balance_t Pike::getWhiteBalance()
    {
    ...
    void Pike::enableAutoShutter(bool value, uint32_t min, uint32_t max)
    {
    ...
    void Pike::enableAutoGain(bool value, uint32_t min, uint32_t max)
    {
    ...
    bool Pike::isAutoGain()
    {
        return (getFeatureMode(DC1394_FEATURE_GAIN) ==
                DC1394_FEATURE_MODE_AUTO);
    }

    void Pike::enableTrigger(bool value)
    {
    ...
    void Pike::setTriggerDelay(unsigned short value)
    {
        setFeatureValue(DC1394_FEATURE_TRIGGER_DELAY, value);
    }

    unsigned short Pike::getTriggerDelay()
    {
        return getFeatureValue<unsigned short>(DC1394_FEATURE_TRIGGER_DELAY);
    }

    void Pike::enableTriggerDelay(bool value)
    {
        enableFeature(DC1394_FEATURE_TRIGGER, value);
    }

    bool Pike::isTriggerDelay()
    {
        return isFeature(DC1394_FEATURE_TRIGGER);
    }
...
//by me
    ...
    void Pike::enableMirror(bool value)
    {
        dc1394error_t err;
    ...
    Pike::lut_info_t Pike::getLUTInfo()
    {
        lut_info_t lutinfo;
        dc1394error_t err;
    ...
    void Pike::enableLUT(bool value)
    {
    ...
    std::vector<uint8_t> Pike::getLUT()
    {
        dc1394error_t err;
    ...
    void Pike::setLUT(std::vector<uint8_t> &lut)
    {
    ...
    void Pike::enableTestImage(bool enable, uint32_t number)
    {
        dc1394error_t err;
    ...
    void Pike::softReset()
    {
        ...
        DC1394_ERR_THROW(err, "uanble to soft reset AVT Pike");

        sleep(5);

        reset();

        pikeInitInternal(this->index);
        init();

        startCapture();
    }

    void Pike::initInternal()
        throw(CameraException)
    {
        if (!this->dc_camera)
        {
        ...
        #ifdef USE_1394B
        std::cout<< "1394B Mode"<<std::endl;
        dc1394_video_set_operation_mode( this->dc_camera, DC1394_OPERATION_MODE_1394B );
    ...
    void Pike::startCaptureInternal()
        throw(CameraException)
    {
    ...
    bool Pike::getFrameInternal(Frame &frame)
    {
